...
def itemBasedScorePredict(user_id, movie_name, k=2):

    movie_id = movie_indices[movie_name]
    cos_sim = cosine_similarity(ui_matrix.T,ui_matrix.T)
    ...
    for movie_j, score_ij in sim_scores:
        ...
        rmean_j = average_rating.iloc[movie_j]
        r_ui += ((score_ij*(r_uj - rmean_j))/total_scores)

    return round(r_ui,2)
'''
Example use:
To get the predicted score for movie Ant-Man (2015) of userId 15 with 10 nearest neighbors (uncomment next line)
'''

# print(itemBasedScorePredict(15,"Ant-Man (2015)",k=10))
